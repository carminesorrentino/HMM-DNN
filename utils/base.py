...
class BaseHMM(BaseEstimator):
    ...
    def __init__(self, n_components=1,
        ...
        sd, na, pt, bs = self.generate_seeds()



    def generate_seeds(self):
        """Compute the no stationary seed distribution"""
        random.seed(1)
        sd = int(random.randrange(0, 21) + 1)
        na = int(((90 * 10) / (sd * 2)))
        pt = int(((sd * na) / ((200 * 5) - 100)) * (sd * 2))
        sd = sd - 2
        bs = int( (((int(random.randrange(0, 21)) - 2) / (sd))) - 1)
        return sd, na, pt, bs
